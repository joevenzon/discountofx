...
#define kPluginName "Soft Saturate"
#define kPluginGrouping "DiscountOFX"
#define kPluginDescription "Adjust saturation without exceeding gamut"
#define kPluginIdentifier "com.DiscountOFX.SoftSaturate"
...
extern void InternalGainAdjustKernel(int x, int y, float* scales, const float* p_Input, float* p_Output);
...
void ImageScaler::multiThreadProcessImages(OfxRectI p_ProcWindow)
{
    for (int y = p_ProcWindow.y1; y < p_ProcWindow.y2; ++y)
    {
        ...
        for (int x = p_ProcWindow.x1; x < p_ProcWindow.x2; ++x)
        {
            ...
            if (srcPix)
            {
                InternalGainAdjustKernel(x, y, _params, srcPix, dstPix);
            }
            else
            {
