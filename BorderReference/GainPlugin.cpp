...
#define kPluginName "Border Reference"
#define kPluginGrouping "DiscountOFX"
#define kPluginDescription "Provide a border with reference black and white"
#define kPluginIdentifier "com.DiscountOFX.BorderReference"
...
enum e_params
{
#define PARAMFLOAT(x, def, name, hint) _e_##x,
...
extern void InternalGainAdjustKernel(int x, int y, int p_Width, int p_Height, float* scales, const float* p_Input, float* p_Output);
...
void ImageScaler::multiThreadProcessImages(OfxRectI p_ProcWindow)
{
    ...
    for (int y = p_ProcWindow.y1; y < p_ProcWindow.y2; ++y)
    {
        ...
        for (int x = p_ProcWindow.x1; x < p_ProcWindow.x2; ++x)
        {
            ...
            if (srcPix)
            {
                InternalGainAdjustKernel(x, y, width, height, _params, srcPix, dstPix);
            }
            else
            {
...
#define PARAMFLOAT(x, def, name, hint) _params[index] = params[index]; index++;
#define PARAMCOUNT(x) 
#include "params.h"
#undef PARAMFLOAT
#undef PARAMCOUNT
...
#define PARAMFLOAT(x, def, name, hint) OFX::DoubleParam* m_param_##x;
#define PARAMCOUNT(x) 
#include "params.h"
#undef PARAMFLOAT
#undef PARAMCOUNT
...
#define PARAMFLOAT(x, def, name, hint) m_param_##x = fetchDoubleParam(#x);
#define PARAMCOUNT(x) 
#include "params.h"
#undef PARAMFLOAT
#undef PARAMCOUNT
...
#define PARAMFLOAT(x, def, name, hint) params[index++] = m_param_##x->getValueAtTime(p_Args.time);
#define PARAMCOUNT(x) 
#include "params.h"
#undef PARAMFLOAT
#undef PARAMCOUNT
...
static DoubleParamDescriptor* defineScaleParam(OFX::ImageEffectDescriptor& p_Desc, const std::string& p_Name, const std::string& p_Label,
                                               const std::string& p_Hint, float default, GroupParamDescriptor* p_Parent)
{
...
void GainPluginFactory::describeInContext(OFX::ImageEffectDescriptor& p_Desc, OFX::ContextEnum /*p_Context*/)
{
    ...
    // Make the components
#define PARAMFLOAT(x, def, name, hint) page->addChild(*defineScaleParam(p_Desc, #x, name, hint, def, componentScalesGroup));
#define PARAMCOUNT(x) 
#include "params.h"
#undef PARAMFLOAT
#undef PARAMCOUNT
